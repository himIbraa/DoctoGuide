...
import 'package:doctoguide/widgets/button.dart';
...
//UPDATE PROFILE
  ...
  Future<void> _updateUserProfile() async {
    ...
    try {
      ...
      final response = await dio.get(
        '$api_endpoint_update_user_profile?id_patient=$_userId&name=$name&email=$email&phone=$phone&gender=$gender&birthdate=$birthDate',
      );

      print('_userId: $_userId, name: $name, email: $email, phone: $phone, gender: $gender, birthDate: $birthDate');
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 30.0),
                InputButton(hintText: 'Phone Number', controller: _phoneController),
                SizedBox(height: 30.0),
                InputButton(hintText: 'Birth Date', controller: _birthDateController),
                SizedBox(height: 30.0),
  ...
  Future<void> _onUpload(String imageUrl) async {
    try {
      ...
      final response = await supabase.from('patient').update({
        'picture': imageUrl,
      }).match({
        'id_user': _userId,
      });
